    ...
    def train(self, source_dataloader_weak, source_dataloader_strong,
        ...
        for epoch in range(start_epoch, epochs):
            ...
            for (data_source_weak, labels_source), (data_source_strong, _), (data_target_weak, _), (data_target_strong, _) in dataset:
                ...
                # print('logits_source_weak',logits_source_weak,logits_source_weak.shape)
                pseudolabels_source = F.softmax(logits_source_weak, 1)
                # print('pseudolabels_source',pseudolabels_source,pseudolabels_source.shape)
                ...
                # print('pseudolabels_target',pseudolabels_target,pseudolabels_target.shape)
                ...
                # print('final_pseudolabels',final_pseudolabels,final_pseudolabels.shape)
                # perform relative confidence thresholding
                row_wise_max, _ = torch.max(pseudolabels_source, dim=1)
                # print('row_wise_max',row_wise_max,row_wise_max.shape)
                ...
                # print('max_values',max_values,max_values.shape)
            ...
            epoch_accuracy_source, labels_list_source, outputs_list_source, preds_list_source = self.evaluate(source_dataloader_weak,return_lists_roc=True)
            epoch_accuracy_target, labels_list_target, outputs_list_target, preds_list_target = self.evaluate(target_dataloader_weak,return_lists_roc=True)
            test_epoch_accuracy, labels_list_test, outputs_list_test, preds_list_test = self.evaluate(target_dataloader_test,return_lists_roc=True)

            # print('source',labels_list_source,preds_list_source)
            # print('target',labels_list_target, preds_list_target)
            # print('test',labels_list_test, preds_list_test)
            ...
            print('[Epoch {}/{}] source_loss:{:.6f} loss: {:.6f}; accuracy source: {:.6f}; accuracy target: {:.6f}; val accuracy: {:.6f};'.format(epoch+1, epochs,source_loss, epoch_loss, epoch_accuracy_source, epoch_accuracy_target, test_epoch_accuracy))
